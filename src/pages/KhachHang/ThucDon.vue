...
import { fetchRecommendDishesByUser } from "@/services/recommender-system";
import {
    fetchGioHang,
    addDishesToCart,
    deleteDishFromCart,
...
// active category
const selectCategory = (maLoai) => {
    activeLoaiMonAn.value = maLoai;
};
...
const goToDishDetail = (dish) => {
    ...
    if (!user || !user.id) {
        return Swal.fire({
            icon: "warning",
            title: "Bạn cần đăng nhập",
            text: "Vui lòng đăng nhập để xem chi tiết món ăn!",
            confirmButtonText: "OK",
        });
    }
...
// Để lưu thông tin tên loại món ăn cho BreadCrumb
const breadcrumbText = computed(() => {
    if (activeLoaiMonAn.value === null) {
        return "";
    }
    const category = danhSachLoaiMonAn.value.find(
        (category) => category.maLoai === activeLoaiMonAn.value
    );
    console.log("Selected Category:", activeLoaiMonAn.value);
    console.log("Breadcrumb Text:", breadcrumbText.value);

    return category ? category.tenLoai : "";
});

//Hàm lấy các món ăn đã lọc (theo loại món ăn hoặc từ khóa tìm kiếm)
const filteredDishes = computed(() => {
    let filtered = [];

    // Nếu người dùng đã đăng nhập, hiển thị các món ăn gợi ý
    if (user && user.id) {
        filtered = danhSachGoiYByUser.value; // Lọc món ăn từ danh sách gợi ý
    } else {
        // Nếu không, hiển thị danh sách món ăn bình thường
        filtered = danhSachMonAn.value;
    }

    if (activeLoaiMonAn.value) {
        filtered = filtered.filter(
            (dish) => dish.maLoai === activeLoaiMonAn.value
        );
    }
...
async function loadData() {
    try {
        const [lmData, monAnData, MAYTData, ghData, gyData] = await Promise.all(
            [
                fetchLoaiMonAn(),
                fetchMonAn(),
                user && user.id
                ...
                user && user.id
                    ? fetchRecommendDishesByUser(user.id)
                    : Promise.resolve([]),
            ]
        );

        // Loại món ăn
        if (Array.isArray(lmData)) {
            danhSachLoaiMonAn.value = lmData;
            activeLoaiMonAn.value = null;
            console.log("danhSachLoaiMonAn:", danhSachLoaiMonAn.value);
        } else {
            console.error("Lỗi dữ liệu khuyến mãi:", lmData);
        }
        ...
        danhSachGoiYByUser.value = rawMonAnData.filter((dish) =>
            gyData.some(
                (recommendedDish) => recommendedDish.maMonAn === dish.maMonAn
            )
        );

        //Giữ thứ từ của `response`
        const recommendedDishIds = gyData.map((dish) => dish.maMonAn);
        // Sắp xếp lại danh sách món ăn gợi ý theo thứ tự trong `response["Mon an goi y"]`
        danhSachGoiYByUser.value = danhSachGoiYByUser.value.sort((a, b) => {
            return (
        ...
        console.log(
            "Danh sách món ăn gợi ý cho user: ",
            danhSachGoiYByUser.value
        );
    } catch (err) {
    ...
    <div class="page-container">
        ...
        <div class="filter-buttons">
            <button
                ...
                :class="{ active: activeLoaiMonAn === null }"
            >
                Tất cả
            </button>
            <button
                class="filter-btn"
                v-for="category in danhSachLoaiMonAn"
                :key="category.maLoai"
                @click="selectCategory(category.maLoai)"
                :class="{ active: activeLoaiMonAn === category.maLoai }"
            >
                {{ category.tenLoai }}
            </button>
        </div>
        ...
        <div class="dishes-list">
            ...
            >
                ...
                <div class="dish-card__body">
                    <h3 class="dish-card__title" @click="goToDishDetail(dish)">
                        {{ dish.tenMonAn }}
                    </h3>
                    <div class="stars-icon">
                        <i
                            v-for="(type, idx) in getStarIcons(dish.avgRating)"
                            :key="idx"
                            :class="{
                    ...
                    <div class="dish-card__sub-body">
                        <div class="dish-card__price">
                            <span class="current">
                                {{
                                    parseInt(dish.donGia).toLocaleString(
                                        "vi-VN"
                                    )
                                }}đ
                            </span>
