...
import { fetchNguyenLieu, addNguyenLieu } from "@/services/nguyenlieu";
import { fetchTaiKhoan } from "@/services/taikhoan";
import { fetchNhaCungCap } from "@/services/nhacungcap";
import { fetchPhieuNhapKho, addPhieuNhapKho } from "@/services/phieunhapkho";
...
async function saveNL() {
    try {
        ...
        // Cập nhật lại danh sách nguyên liệu mới lấy từ API
        danhSachNguyenLieu.value = await fetchNguyenLieu();

        // Tìm nguyên liệu vừa thêm vào danh sách
        const addedNguyenLieu = danhSachNguyenLieu.value.find(
            (nl) => nl.maNguyenLieu === response.maNguyenLieu
        );

        // Nếu tìm thấy nguyên liệu vừa thêm, thêm vào danh sách đã chọn
        if (addedNguyenLieu) {
            addNguyenLieuToList(addedNguyenLieu); // Thêm vào danh sách chọn
        }
...
const submitNhapKho = async () => {
    if (!selectedMaNCC.value || !selectedMaTaiKhoan.value) {
        return Swal.fire(
            "Thiếu dữ liệu",
            "Vui lòng chọn NCC và nhân viên",
            "warning"
        );
    }
    ...
    try {
        ...
        Swal.fire("Thành công", "Nhập kho thành công", "success");
        router.push({ name: "LichSuNhapKho" });
    } catch (err) {
        console.error(err);
        Swal.fire("Lỗi", "Không thể lưu phiếu nhập kho", "error");
    }
};
...
<template>
    <div class="page-container">
        ...
        <div class="top-bar">
            ...
            <div class="page-title">Thêm phiếu nhập kho mới</div>
        </div>
        ...
        <div class="ingredients-container">
            <div class="left-panel">
                ...
                <div class="add-new-ingredient" @click="onAddNew">
        ...
        <div class="form-actions">
            <button type="button" class="btn-save" @click="submitNhapKho">
                Lưu
            </button>
        ...
        <div v-if="showModal" class="modal" @click="closeModalOnClickOutside">
            <div class="modal-content" @click.stop>
                ...
                <div class="modal-footer">
                    <button class="btn-save" @click="saveNL()">Lưu</button>
                    <button class="btn-cancel" @click="resetModal()">
                        Hủy
                    </button>
                </div>
            </div>
...
.add-new-ingredient i {
    margin-right: 6px;
}

.add-new-ingredient:hover {
    text-decoration: underline;
}
